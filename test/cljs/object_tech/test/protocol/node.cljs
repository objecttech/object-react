(ns object-tech.test.protocol.node
  (:require [clojure.string :as s]
            [object-tech.test.protocol.utils :as utils]))
...
(defn prepare-env! []
  ...
  (let [dir  (s/join "/" [build-dir "status-go"])
    ...
    (if-not (utils/exist? dir)
      (utils/exec-sync "git clone https://github.com/status-tech.object-go.git -b bug/whisper-on-geth1.6.1" #js {:cwd build-dir})
