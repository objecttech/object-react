(ns objecttech.contacts.events
  (:require [re-frame.core :refer [dispatch reg-fx reg-cofx inject-cofx]]
            [objecttech.utils.handlers :refer [register-handler-db register-handler-fx]]
            [objecttech.data-store.contacts :as contacts]
            [objecttech.utils.crypt :refer [encrypt]]
            [clojure.string :as s]
            ...
            [objecttech.contacts.db :as v]
            [objecttech.contacts.navigation]))
...
(reg-fx
  ...
  (fn [{:keys [web3 current-public-key name whisper-identity photo-path current-account-id object
...
(defn- update-pending-object [old-contacts {:keys [whisper-identity pending?] :as contact}]
  ...
  (fn [{:keys [db]} [_ new-contacts]]
    (let [{:contacts/keys [contacts]} db
          ...
          new-contacts' (->> new-contacts
                             (map #(update-pending-object contacts %))
  ...
  (fn [{{:keys [accounts current-account-id] :as db} :db} [_ contact]]
    (let [current-account (get accounts current-account-id)]
      {::send-contact-request-fx (merge
                                   ...
                                   (select-keys current-account [:name :photo-path :object
    ...
    (let [{:keys [chats current-public-key]} db]
      (when (not= current-public-key from)
        (let [{:keys [content timestamp]} payload
              {:keys [object name profile-image]} (:profile content)
