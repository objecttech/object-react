(ns objecttech.transactions.handlers
  (:require [re-frame.core :refer [after dispatch debug enrich]]
            ...
            [objecttech.utils.hex :refer [valid-hex? normalize-hex]]
...
(register-handler :accept-transactions
  (u/side-effect!
   (fn [{:keys [transactions]} [_ password]]
     ...
     (object/complete-transactions (keys transactions) password on-transactions-completed))))

(register-handler :accept-transaction
  (u/side-effect!
   (fn [_ [_ password id]]
     (dispatch [:set :wrong-password? false])
     (object/complete-transactions (list id) password on-transactions-completed))))
