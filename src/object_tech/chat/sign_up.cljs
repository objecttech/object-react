(ns objecttech.chat.sign-up
  (:require [re-frame.core :refer [subscribe dispatch dispatch-sync]]
            ...
            [objecttech.utils.sms-listener :refer [add-sms-listener
                                                  remove-sms-listener]]
...
(defn on-send-code-response [body]
  (dispatch [:received-message
             {:message-id   (random/id)
              ...
              :from         console-chat-id
              :to           "me"}])
  (let [object (keyword (:object body))]
    (when (= :confirmed object)
...
(defn intro []
  (dispatch [:received-message intro-object])
