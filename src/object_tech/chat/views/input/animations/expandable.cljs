(ns objecttech.chat.views.input.animations.expandable
  ...
  (:require [reagent.core :as r]
            ...
            [objecttech.components.drag-drop :as drag]
            [objecttech.components.react :refer [view
                                                animated-view]]
            [objecttech.chat.views.input.animations.responder :as resp]
            [objecttech.chat.views.input.utils :as input-utils]
            [objecttech.chat.styles.animations :as style]
      ...
      {:component-did-mount
       ...
       (fn [{:keys [draggable? custom-header]} & elements]
         ...
         (let [{expandable-offset :expandable-offset
                object-bar-height :height} (get-in p/platform-specific [:component-styles :object-bar :main])
               bottom (+ @input-height @chat-input-margin)
               height (if @fullscreen?
                        (+ @max-height object-bar-height expandable-offset)
                        anim-value)]
