(ns objecttech.chat.handlers.send-message
  (:require [objecttech.utils.handlers :refer [register-handler] :as u]
            ...
            [objecttech.chat.constants :refer [input-height]]
            [objecttech.utils.datetime :as datetime]
            [objecttech.protocol.core :as protocol]
            [taoensso.timbre :refer-macros [debug] :as log]
            [objecttech.chat.handlers.console :as console]
            [objecttech.utils.types :as types]
...
(register-handler :prepare-command!
  (u/side-effect!
    (fn [{:keys [current-public-key network-object] :as db}
      ...
      (let [clock-value   (messages/get-last-clock-value chat-id)
        ...
        (dispatch [:update-message-overhead! chat-id network-object])
        (dispatch [:set-chat-ui-props {:sending-in-progress? false}])
...
(register-handler :prepare-message
  (u/side-effect!
    (fn [{:keys [network-object] :as db} [_ {:keys [chat-id identity message] :as params}]]
...
(register-handler ::send-message!
  (u/side-effect!
    (fn [{:keys [web3 chats network-object current-account-id accounts]
          ...
          (when message
            (let [message' (select-keys message [:from :message-id])
                  payload  (select-keys message [:timestamp :content :content-type
                                                 :clock-value :show?])
                  payload  (if (= network-object :offline)
...
(register-handler ::send-command-protocol!
  (u/side-effect!
    (fn [{:keys [web3 current-public-key chats network-object
