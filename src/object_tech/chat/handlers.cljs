(ns objecttech.chat.handlers
  (:require-macros [cljs.core.async.macros :as am])
  (:require [re-frame.core :refer [enrich after debug dispatch reg-fx]]
            [objecttech.models.commands :as commands]
            [clojure.string :as string]
            [objecttech.components.styles :refer [default-chat-color]]
            [objecttech.chat.models.suggestions :as suggestions]
            ...
            [objecttech.chat.sign-up :as sign-up-service]
            [objecttech.navigation.handlers :as nav]
            [objecttech.utils.handlers :refer [register-handler register-handler-fx] :as u]
            [objecttech.handlers.server :as server]
            [objecttech.utils.phone-number :refer [format-phone-number
                                                  valid-mobile-number?]]
            [objecttech.components.object :as object]
            [objecttech.utils.types :refer [json->clj]]
            [objecttech.chat.utils :refer [console? not-console? safe-trim]]
            [objecttech.utils.gfycat.core :refer [generate-gfy]]
            objecttech.chat.handlers.input
            objecttech.chat.handlers.commands
            objecttech.chat.handlers.animation
            objecttech.chat.handlers.requests
            objecttech.chat.handlers.unviewed-messages
            objecttech.chat.handlers.send-message
            objecttech.chat.handlers.receive-message
            objecttech.chat.handlers.faucet
            [cljs.core.async :as a]
            objecttech.chat.handlers.webview-bridge
            objecttech.chat.handlers.console
...
(defmethod nav/preload-data! :chat
  ...
  (let [chat-id           (or id current-chat-id)
       ...
       (dispatch [:load-requests! chat-id])
        ;; todo rewrite this. temporary fix for https://github.com/objecttech/object-react/issues/607
...
(register-handler :update-message-overhead!
  (u/side-effect!
    (fn [_ [_ chat-id network-object]]
      (if (= network-object :offline)
