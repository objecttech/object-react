  ...
  (->interceptor
    ...
    (fn check-handler
      ...
      (let [new-db (get-effect context :db)
        ...
        (when (and new-db (not (s/valid? :objecttech.specs/db new-db)))
          (throw (ex-info (str "spec check failed on: " (first v) "\n " (s/explain-str :objecttech.specs/db new-db)) {})))
        context))))
