(ns objecttech.data-store.messages
  (:require [objecttech.data-store.realm.messages :as data-store]
            [clojure.string :refer [join split]]
            [objecttech.utils.random :refer [timestamp]]
            [objecttech.utils.utils :refer [update-if-present]]
            [clojure.walk :refer [stringify-keys keywordize-keys]]
            [cljs.reader :refer [read-string]]
            [objecttech.constants :as c])
  (:refer-clojure :exclude [update]))

(defn- user-objectes-to-map
  [user-objectes]
  (->> (vals user-objectes)
       (mapv (fn [{:keys [whisper-identity] :as object}]
               [whisper-identity object]))
       (into {})))
...
(defn get-by-id
  ...
  (some-> (data-store/get-by-id message-id)
          (clojure.core/update :user-objectes user-objectes-to-map)))
...
(defn get-messages
  [messages]
  (->> messages
       (mapv #(clojure.core/update % :user-objectes user-objectes-to-map))
       (into '())
       reverse
...
(defn update 
  ...
  (when (data-store/exists? message-id)
    (let [message (update-if-present message :user-objectes vals)]
      (data-store/save message))))
