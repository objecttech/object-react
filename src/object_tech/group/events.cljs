(ns objecttech.group.events
  (:require [objecttech.protocol.core :as protocol]
            ...
            [objecttech.data-store.contact-groups :as groups]
            [clojure.string :as string]
            [objecttech.utils.random :as random]
            [objecttech.group.navigation]))
...
(defn update-pending-object [old-groups {:keys [group-id pending?] :as group}]
  ...
  (fn [{{:group/keys [contact-groups] :as db} :db} [_ new-groups]]
    (let [identities  (set (keys contact-groups))
          ...
          new-groups' (->> new-groups
                           (map #(update-pending-object contact-groups %))
